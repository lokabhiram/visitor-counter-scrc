...
def get_visitor_count():
    return "Number of Visitors:" + str(cnt_down)
...
if __name__ == '__main__':
    ...
    while (cap.isOpened()):
                    ...
                    for i in persons:
                        if abs(x - i.getX()) <= w and abs(y - i.getY()) <= h:
                            ...
                            if i.going_UP(line_down, line_up):
                                ...
                                log.write("ID: " + str(i.getId()) + ' crossed going up at ' + time.strftime(
                                    "%c") + '\n')
                            elif i.going_DOWN(line_down, line_up):
                                # print("ID:", i.getId(), 'crossed going down at', time.strftime("%c"))
                                log.write("ID: " + str(i.getId()) + ' crossed going down at ' + time.strftime(
                                    "%c") + '\n')
                            break
                        if i.getState() == '1':
        ...
        if current_time - start_time >= 1:  # Check if one second has passed
            ...
            save_visitor_count()  # Save visitor count to file
            prev_cnt = cnt_down
